...
async def after_playing(error, voice_client, ctx):
    if ctx is None:
        return
    ...
    if inqueue:
        ...
        voice_client.play(next_player, after=lambda error: bot.loop.create_task(after_playing(error, voice_client, ctx)))
    else:
...
async def play(ctx):
    if hasperms(ctx):
        try:
            ...
            voice_client.play(player, after=lambda error: bot.loop.create_task(after_playing(error, voice_client, ctx)))
